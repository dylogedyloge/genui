...
const FlightCard = ({
  ...
  const [isAccordionOpen, setIsAccordionOpen] = useState(false); // State for accordion
  const [flightInfo, setFlightInfo] = useState(null); // State to store flightInfo
  const [baggageRules, setBaggageRules] = useState(null); // State for baggage rules
  const [refundRules, setRefundRules] = useState(null); // State for refund rules
  const [isLoadingBaggage, setIsLoadingBaggage] = useState(false); // State for baggage rules loading
  const [isLoadingRefund, setIsLoadingRefund] = useState(false); // State for refund rules loading
  ...
  const convertToJalali = (
    ...
    if (isDomestic) {
      const extractedTime = time && typeof time === "string" && time.includes("-")
        ? time.split("-").pop()?.trim()
        : time;
  ...
  useEffect(() => {
    if (isAccordionOpen && !isDomestic) {
      // Fetch baggage rules
      setIsLoadingBaggage(true);
      fetch(`${API_ENDPOINTS.INTERNATIONAL.BAGGAGE}?fare_source_code=${fareSourceCode}`)
        .then((response) => response.json())
        .then((data) => {
          setBaggageRules(data.data.baggage_info);
          setIsLoadingBaggage(false);
      ...
      // Fetch refund rules
      setIsLoadingRefund(true);
      fetch(`${API_ENDPOINTS.INTERNATIONAL.RULES}?fare_source_code=${fareSourceCode}`)
        .then((response) => response.json())
        .then((data) => {
          setRefundRules(data.data);
          setIsLoadingRefund(false);
  ...
  return (
    <Card className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="font-bold text-lg">{getPrice().toLocaleString()} ریال</span>
      </div>
      ...
      {!isDomestic && (
        <Button
          ...
          className="w-full"
        >
          {isAccordionOpen ? "بستن جزئیات" : "نمایش جزئیات"}
        </Button>
      )}
          ...
          <AccordionItem value="details">
            <AccordionContent className="space-y-4">
              ...
              <Tabs defaultValue="flight-details">
                ...
                <TabsContent value="flight-details">
                  <div className="space-y-2">
                    {[
                      ...
                      { label: "شماره پرواز", value: flightDetails.flightNumber },
                      ...
                      { label: "قیمت به ریال", value: getPrice().toLocaleString() },
                      { label: "کلاس پرواز", value: cabin.persian },
                      { label: "بار مجاز", value: flightDetails.baggage },
                      { label: "مدت پرواز", value: flightDetails.flightDuration },
                    ].map((item, index) =>
                      item.value && (
                ...
                <TabsContent value="baggage-rules">
                  {isLoadingBaggage ? (
                    <div>در حال بارگذاری قوانین بار...</div>
                  ) : baggageRules ? (
                    Array.isArray(baggageRules) ? (
                      <ul>
                        {baggageRules.map((rule: any, index: number) => (
                          <li key={index}>
                            {rule.departure} به {rule.arrival}: {rule.baggage}
                          </li>
                        ))}
                      </ul>
                    ) : (
                    ...
                    <div>داده‌ای یافت نشد.</div>
                  )}
                </TabsContent>

                {/* Refund Rules Content */}
                <TabsContent value="refund-rules">
                  {isLoadingRefund ? (
                    <div>در حال بارگذاری قوانین استرداد...</div>
                  ) : refundRules ? (
                    Array.isArray(refundRules) ? (
                      <ul>
                        {refundRules.map((rule: any, index: number) => (
                          <li key={index}>
                            ...
                            {renderMarkdown(rule.rules_parsed)}
                          </li>
                        ))}
                      </ul>
                    ) : (
